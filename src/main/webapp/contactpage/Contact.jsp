				...
				<div class="container">
					<div class="heading_container">
						...
						<div class="button-container">
							<td width="100">
								<button type="button"
									onclick="location.href='./contactwrite.do';" class="btn_course">글쓰기</button>
							</td>
						</div>
					</div>
										...
										<c:forEach items="${ boardLists }" var="row" varStatus="loop">
											<c:if test="${row.type == 'data' }">
												<tr align="center">
													...
													<td align="left"><a
														href="./contactview.do?idx=${ row.idx }"> ${ row.title }
													</a></td>
													<td>${ row.id }</td>
													<td>${ row.postdate }</td>
													...
													<!-- 공백만들면 안된다.  -->
											            <c:if test="${ not empty row.ofile }">
											            	<a href="./contanctdownload.do?ofile=${ row.ofile }&sfile=${ row.sfile }&idx=${ row.idx }">
											            	<img src="./images/download1.jpg" alt="다운로드" width="30" height=30></a>
											            </c:if>
