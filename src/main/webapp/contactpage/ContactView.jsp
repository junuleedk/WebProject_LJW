	...
	<section class="course_section layout_padding">
		...
		<div class="container">
			<div class="heading_container">
				<h3>Contact Us Q&A BOARD</h3>
				<p>If you have any questions for us, please don't hesitate to ask us any time.</p>
			</div>
			...
			<table class="board_table">
				...
				<tr>
					<td>내용</td>
					<td colspan="3" height="100">${ dto.content }<br><br>
						<c:if test="${ not empty dto.sfile }">
							...
							<c:forTokens items="${ imageType }" delims="," var="type">
								<c:if test="${ fileType == type }">
           							<img src="Uploads/${dto.sfile}" style="max-width:500px;" />
        						</c:if>
							</c:forTokens>
							<c:forTokens items="${ videoType }" delims="," var="type">
								<c:if test="${ fileType == type }">
           							<video src="Uploads/${ dto.sfile }" autoplay controls></video>
        						</c:if>
							</c:forTokens>
							<c:forTokens items="${ audioType }" delims="," var="type">
								<c:if test="${ fileType == type }">
           							<audio src="Uploads/${ dto.sfile }" autoplay controls></audio>
        						</c:if>
							</c:forTokens>
						</c:if>
					</td>
				...
				<tr>
					...
					<td>
						...
						<c:if test="${ not empty dto.ofile }">${ dto.ofile }
           			 		<a href="./contanctdownload.do?ofile=${ dto.ofile }&sfile=${ dto.sfile }&idx=${ dto.idx }">
								[다운로드] </a>
						</c:if>
					</td>
					<td>다운로드수</td>
				...
				<tr>
					<td colspan="4" style="text-align: center;"><c:if
							test="${ dto.id eq userId}" var="result">
							<button type="button" class="btn_course"
								onclick="location.href='./contactedit.do?mode=edit&idx=${ param.idx}';">
								수정하기</button>
							<button type="button" class="btn_course"
								onclick="location.href='./contactdelete.do?mode=delete&idx=${ param.idx}';">
								삭제하기</button>
							<button type="button" class="btn_course"
